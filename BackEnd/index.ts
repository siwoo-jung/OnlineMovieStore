...
app.post('/login', async (req:any, res:any) => {
    try {
        ...
        sql.connect(config, () => {
            ...
            validRequest.query(validQuery, (err:any, rows:any) => {
                if (err) {
                    console.log(err);
                } else {
                    if (rows.recordset[0]) {
...
app.post('/signup', async (req: any, res :any) => {
    try {
        ...
        if (!username || !password) {
            ...
            request.query(query, (err:any, rows:any) => {
                if (err) {
                    console.log(err);
                } else {
                    ...
                    if (rows.recordset[0]) {
                        ...
                        userDbUpdateRequest.query(userDbUpdateQuery, (err:any, rows:any) => {
                            if (err) {
                                console.log(err);
                            } else {
                        ...
                        userWatchlistRequest.query(userWatchlistQuery, (err:any, rows:any) => {
                            if (err) {
                                console.log(err);
                            } else {
...
app.post('/movieadd', async (req: any, res: any) => {
        ...
        request.query(query, (err: any, rows: any) => {
            if (err) {
                ...
                if (rows.recordset[0]) {
                    ...
                    var request = new sql.Request();
                    request.query(query, (err2: any, rows2: any) => {
                        if (err2) {
                            console.log(err2);
...
app.delete('/moviedelete', async (req: any, res: any) => {
    sql.connect(config, () => {
        const movieTitle = req.body.movieTitle;
        const movieYear = req.body.movieYear;
        const movieRuntime = req.body.movieRuntime;
        ...
        request.query(query, (err: any, rows: any) => {
            if (err) {
                ...
                if (rows.recordset[0]) {
                    ...
                    deleteRequest.query(deleteQuery, (err2: any, rows2: any) => {
                        if (err2) {
                            console.log(err2);
                            return res.status(500).json({error: 'Server Error'});
                        } else {
...
app.put('/movieupdate', async (req: any, res: any) => {
    sql.connect(config, () => {
        const movieTitle = req.body.movieTitle;
        const movieYear = req.body.movieYear;
        const movieRuntime = req.body.movieRuntime;
        ...
        request.query(query, (err: any, rows: any) => {
            if (err) {
                ...
                if (rows.recordset[0]) {
                    ...
                    updateRequest.query(updateQuery, (err2: any, rows2: any) => {
                        if (err2) {
                            console.log(err2);
                            return res.status(500).json({error: 'Server Error'});
                        } else {
...
app.get('/viewwatchlist/:name', async (req: any, res: any) => {
    sql.connect(config, () => {
...
app.post('/addwatchlist/:name', async (req:any, res:any) => {
    sql.connect(config, () => {
        ...
        searchRequest.query(searchQuery, (err: any, rows:any) => {
            if (err) {
                console.log(err);
                return res.status(500).send('Server Error');
            } else {
                if (rows.recordset[0]) {
                    ...
                    searchWatchlistRequest.query(searchWatchlistQuery, (err:any, rows:any) => {
                        if (err) {
                            console.log(err);
                            return res.status(500).send('Server Error');
                        } else {
                            ...
                            if (rows.recordset[0]) {
                                ...
                                addRequest.query(addQuery, (err: any, rows: any) => {
                                    if (err) {
                                        console.log(err);
                                        return res.status(500).send('Server Error');
                                    } else {
                    ...
                    return res.status(400).json(
                        {error: "Cannot add this movie to the watchlist"})
                }
            }
        })
    })
...
app.delete('/deletewatchlist/:name', async (req:any, res:any) => {
    sql.connect(config, () => {
        const username = req.params.name;
        const movieTitle = req.body.movieTitle;
        ...
        searchRequest.query(searchQuery, (err:any, rows:any) => {
            if (err) {
                ...
                if (rows.recordset[0]) {
                    ...
                    deleteRequest.query(deleteQuery, (err:any, row:any) => {
                        if (err) {
                            console.log(err);
                            return res.status(500).send('Server Error');
                        } else {
...
app.put('/checkpassword/:name', async (req: any, res: any) => {
    sql.connect(config, () => {
        const username = req.params.name;
...
// Change password
app.put('/changepassword/:name', async (req: any, res: any) => {
    sql.connect(config, () => {
        const username = req.params.name;
        const newPassword = req.body.newPassword;
        const updateQuery = `UPDATE UserInfo SET password='${newPassword}' WHERE username='${username}'`;
        const updateRequest = new sql.Request();
        updateRequest.query(updateQuery, (err:any, rows:any) => {
            if (err) {
                console.log(err);
                return res.status(500).send('Server Error');
            } else {
...
app.get('/browseusers', async (req: any, res: any) => {
    sql.connect(config, () => {
...
app.delete('/deleteuser', async (req: any, res: any) => {
    sql.connect(config, () => {
        ...
        checkRequest.query(checkQuery, (err:any, rows:any) => {
            if (err) {
                ...
                deleteUserRequest.query(deleteUserQuery, (err:any, rows:any) => {
                    if (err) {
                        console.log(err.message);
                        return res.status(500).send('Server Error');
                    } else {
                ...
                deleteWatchlistRequest.query(deletWatchlistQuery, (err:any, rows:any) => {
                    if (err) {
                        console.log(err.message);
                        return res.status(500).send('Server Error');
                    } else {
...
app.put('/authorization', async (req:any, res:any) => {
    const targetUser = req.body.targetUser;
    ...
    checkRequest.query(checkQuery, (err:any, rows:any) => {
        if (err) {
            ...
            changeRequest.query(changeQuery, (err:any, rows:any) => {
                if (err) {
                    console.log(err.message);
                    return res.status(500).send('Server Error');
                } else {
